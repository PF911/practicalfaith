...
function openLevelMenu(category){
  ...
  const unseenBtn = document.getElementById('unseenBtn');
  if(unseenBtn && typeof quizQuestions !== 'undefined'){
    const progress = loadProgress();
    const answered = progress[category] || {};
    const hasUnseen = quizQuestions.some(q => q.game === category && !answered[String(q.id)]);
    unseenBtn.style.display = hasUnseen ? 'flex' : 'none';
  }
...
function toggleWhy(){
  ...
  if(btn) btn.textContent = panel.classList.contains('open') ? '💡 Hide' : '💡 Why?';
}
window.toggleWhy = toggleWhy;

function toggleDailyWhy(){
  const panel = document.getElementById('dailyWhyPanel');
  const btn   = document.getElementById('dailyWhyBtn');
  if(!panel) return;
  panel.classList.toggle('open');
  if(btn) btn.textContent = panel.classList.contains('open') ? '💡 Hide' : '💡 Why?';
}
window.toggleDailyWhy = toggleDailyWhy;

// Also reset daily explanation on new question
const _origLoadDailyQuestion = loadDailyQuestion;
// (daily reset handled inline in loadDailyQuestion below)
...
function startUnseenMode(){
  if(typeof quizQuestions === 'undefined' || !quizQuestions.length) return;
  const progress  = loadProgress();
  const answered  = progress[currentCategory] || {};
  const pool = shuffleArray(
    quizQuestions.filter(q => q.game === currentCategory && !answered[String(q.id)])
  ).slice(0, QUESTIONS_PER_ROUND);

  if(!pool.length){
    alert("You've answered every question in this category!

Try Practice Mode to review the tough ones.");
    return;
  }
...
window.startUnseenMode = startUnseenMode;
...
function loadQuestion(){
  ...
  // Reset explanation panel for each new question
  ...
  if(whyPanel){ whyPanel.textContent = ''; whyPanel.classList.remove('open'); }
  if(whyBtn)  whyBtn.textContent = '💡 Why?';
  renderQuestion();
  startTimer();
}
...
function submitAnswer(index){
  ...
  if(index === q.answer){
    ...
    // Explanation field (option C: collapsed Why? button)
    const whyRow = document.getElementById('whyRow');
    const whyPanel = document.getElementById('whyPanel');
    const whyBtn  = document.getElementById('whyBtn');
    if(whyRow && whyPanel && whyBtn){
      if(q.explanation){
        whyPanel.textContent = q.explanation;
        whyPanel.classList.remove('open');
        whyBtn.textContent = '💡 Why?';
        whyRow.style.display = 'block';
      } else {
  ...
  // Explanation field on wrong answer too
  const whyRowW = document.getElementById('whyRow');
  const whyPanelW = document.getElementById('whyPanel');
  const whyBtnW  = document.getElementById('whyBtn');
  if(whyRowW && whyPanelW && whyBtnW){
    if(q.explanation){
      whyPanelW.textContent = q.explanation;
      whyPanelW.classList.remove('open');
      whyBtnW.textContent = '💡 Why?';
      whyRowW.style.display = 'block';
    } else {
      whyRowW.style.display = 'none';
    }
  }
...
function pickDailyQuestions(){
  ...
  // Seeded LCG so every question is equally reachable and
  // cross-category combinations are genuinely random yet deterministic.
  let seed = 0;
  ...
  // Partial Fisher-Yates: shuffle indices, take first 5
  const indices = quizQuestions.map((_, i) => i);
  for(let i = indices.length - 1; i > 0; i--){
    const j = Math.floor(nextRand() * (i + 1));
    [indices[i], indices[j]] = [indices[j], indices[i]];
  }
  return indices.slice(0, 5).map(i => quizQuestions[i]);
}
...
function loadDailyQuestion(){
  ...
  // Reset daily explanation panel for each new question
  const dWhyRow2 = document.getElementById('dailyWhyRow');
  const dWhyPanel2 = document.getElementById('dailyWhyPanel');
  const dWhyBtn2  = document.getElementById('dailyWhyBtn');
  if(dWhyRow2)   dWhyRow2.style.display = 'none';
  if(dWhyPanel2){ dWhyPanel2.textContent = ''; dWhyPanel2.classList.remove('open'); }
  if(dWhyBtn2)   dWhyBtn2.textContent = '💡 Why?';
...
function submitDailyAnswer(index){
  ...
  // Explanation field for daily card
  const dWhyRow = document.getElementById('dailyWhyRow');
  const dWhyPanel = document.getElementById('dailyWhyPanel');
  const dWhyBtn  = document.getElementById('dailyWhyBtn');
  if(dWhyRow && dWhyPanel && dWhyBtn){
    if(q.explanation){
      dWhyPanel.textContent = q.explanation;
      dWhyPanel.classList.remove('open');
      dWhyBtn.textContent = '💡 Why?';
      dWhyRow.style.display = 'block';
    } else {
      dWhyRow.style.display = 'none';
    }
  }
...
const tutorialSlides = [
  {
    ...
    body: 'Test your Bible knowledge with 1,100+ questions across 15 categories — from Trivia and Prophecy to Parables and Timeline events.'
  },
  {
    icon: '🎓',
